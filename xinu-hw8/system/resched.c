...
syscall resched(void)
{
    ...
    {
        if(!isempty(readylist[cpuid][PRIORITY_MED]))
        {
       // enqueue(dequeue(currpid[cpuid], readylist[cpuid][PRIORITY_HIGH]));
        ...
        if(!isempty(readylist[cpuid][PRIORITY_LOW]))
        {
            enqueue(dequeue(readylist[cpuid][PRIORITY_LOW]),readylist[cpuid][PRIORITY_MED]);
        }
        //enqueue(dequeue(currpid[cpuid], readylist[cpuid][PRIORITY_MED]));
