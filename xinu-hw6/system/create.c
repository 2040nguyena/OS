...
extern int _atomic_increment_increment(int *);
...
syscall create(void *funcaddr, ulong ssize, ulong priority, char *name, ulong nargs, ...)
{
	...
	//saddr = (ulong *)getmem(ssize);  //Make sure to have getmem working before changing this line
	...
	_atomic_increment_increment(&numproc);
