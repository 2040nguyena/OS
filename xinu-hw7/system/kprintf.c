 ...
 *      to an <code>int</code>.
 */
syscall kgetc(void) //from buffer(global array) or dr
{
 ...
 */
syscall kcheckc(void)//checks buffers or dr
{
    volatile struct pl011_uart_csreg *regptr;
    regptr = (struct pl011_uart_csreg *)0x3F201000;
...
{
    // TODO: Check for room in unget buffer, put the character in or discard. hint = create gloabl array
    int i = 0;
    if(i < UNGETMAX)
    {
        ungetArray[i++] = c;
